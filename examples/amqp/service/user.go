...
func (s *UserService) ReceiveFanout() {
	for {
		_, err := s.channel.QueueDeclare("test", false, false,
				false, false, nil)

		err = s.channel.QueueBind("test", "info", exchange, false, nil)
		msg, ok, err := s.channel.Get("test", true)
		if !ok {
			fmt.Println("do not get msg")
			time.Sleep(3*1e9)
			continue
...
func (s *UserService) ReceiveFanout3() {
	for {
